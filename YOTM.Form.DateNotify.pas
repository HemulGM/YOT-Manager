...
uses
  ...
  YOTM.Form.ModalEdit, HGM.Button, Vcl.StdCtrls, Vcl.ExtCtrls, Vcl.WinXCalendars,
  YOTM.Main, YOTM.DB.Tasks, Vcl.Menus, HGM.Common.Utils, HGM.Controls.PanelExt;
...
uses
  YOTM.Form.DateNotify.TaskRepeat;
...
begin
  case FData.SelectRepeatType of
    ttSimple:
      ButtonFlatRepeat.Caption := 'Не повторять';
    ttRepeatInDay:
      ButtonFlatRepeat.Caption := 'Ежедневно';
    ttRepeatInWeek:
      ButtonFlatRepeat.Caption := 'Еженедельно';
    ttRepeatInMonth:
      ButtonFlatRepeat.Caption := 'Ежемесячно';
    ttRepeatInYear:
      ButtonFlatRepeat.Caption := 'Ежегодно';
  end;
end;
...
begin
  case ButtonFlatSelectTime.ImageIndex of
    ...
    10:
      begin
        if PanelSelectTime.Visible then
          PanelSelectTime.Hide
        else
        begin
...
begin
  if EditTime.Text = '' then
    ButtonFlatSelectTime.ImageIndex := 10
  else
    ButtonFlatSelectTime.ImageIndex := 0;
end;
