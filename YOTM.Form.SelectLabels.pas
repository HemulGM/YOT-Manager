...
uses
  ...
  YOTM.Form.ModalEdit, Vcl.ExtCtrls, HGM.Button, Vcl.StdCtrls, Vcl.Grids,
  HGM.Controls.VirtualTable, YOTM.DB.LabelTypes, YOTM.DB, YOTM.DB.Labels,
...
type
  TFormSelectLabels = class(TFormModalEdit)
    ...
    procedure TableExLabelsDrawCellData(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
    procedure ButtonFlatDeleteClick(Sender: TObject);
    procedure TableExLabelsItemColClick(Sender: TObject; MouseButton: TMouseButton; const Index: Integer);
...
begin
  ...
  begin
    if not IndexInList(TableExLabels.ItemIndex, FLabelTypes.Count) then
      Exit;
    if TFormAnswer.GetAnswer('Удалить выбранную метку?') then
      FLabelTypes.Delete(TableExLabels.ItemIndex);
  end;
end;
...
procedure TFormSelectLabels.FormDestroy(Sender: TObject);
begin
  FLabelTypes.Clear;
  FLabelTypes.Free;
  inherited;
...
begin
  if not IndexInList(ARow, FLabelTypes.Count) then
    Exit;
  if ACol = 1 then
  ...
  if ACol = 0 then
    with TableExLabels.Canvas do
    begin                            //18 t 19 f
      FormMain.ImageList24.Draw(TableExLabels.Canvas, Rect.Left + (Rect.Width div 2 - FormMain.ImageList24.Width div 2), Rect.Top + (Rect.Height div 2 - FormMain.ImageList24.Height div 2), CheckImage(FLabelTypes.Checked[ARow]));
    end;
end;

procedure TFormSelectLabels.TableExLabelsGetData(FCol, FRow: Integer; var Value: string);
begin
  if not IndexInList(FRow, FLabelTypes.Count) then
    Exit;
  Value := '';
  case FCol of
    2:
      Value := FLabelTypes[FRow].Name;
  end;
end;
...
begin
  ...
  case Index of
    0:
      FLabelTypes.Checked[TableExLabels.ItemIndex] := not FLabelTypes.Checked[TableExLabels.ItemIndex];
  end;
end;
