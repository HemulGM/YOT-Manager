...
type
   ...
   TFormTaskRepeat = class(TFormModalEdit)
    ...
    procedure DrawGridMonthsDrawCell(Sender: TObject; ACol, ARow: Integer;
      Rect: TRect; State: TGridDrawState);
    procedure FormDestroy(Sender: TObject);
    procedure DrawGridMonthsMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure DrawGridMonthsMouseLeave(Sender: TObject);
    procedure DrawGridMonthsMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure DrawGridMonthsMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
   ...
   if FCalState.MouseActive then
    begin
     if FCalState.MouseCord = Point(ACol, ARow) then
      if FCalState.MouseDown then Brush.Color:=$00616161 else Brush.Color:=$003A3A3A;
    end;
   if FCalendar[i-1].Checked then Brush.Color:=$00616161;
...
procedure TFormTaskRepeat.DrawGridMonthsMouseDown(Sender: TObject;
...
begin
 case FRepeatType of
  ttSimple: FRepeatString:='';
  ttRepeatInDay: FRepeatString:='';
  ttRepeatInWeek:
   begin
    FRepeatString:='';
    for i:=1 to 7 do FRepeatString:=FRepeatString + IntToStr(Ord(WDays[i]));
   end;
  ttRepeatInMonth:
   begin
    FRepeatString:='';
    for i:=1 to 12 do FRepeatString:=FRepeatString + IntToStr(Ord(FCalendar[i-1].Checked));
   end;
  ttRepeatInYear: FRepeatString:='';
 end;
...
begin
 if Tab = TabSheetEmpty then FRepeatType:=ttSimple
 else
 if Tab = TabSheetDay then FRepeatType:=ttRepeatInDay
 else
 if Tab = TabSheetWeek then FRepeatType:=ttRepeatInWeek
 else
 if Tab = TabSheetMonth then FRepeatType:=ttRepeatInMonth
 else
 if Tab = TabSheetYear then FRepeatType:=ttRepeatInYear
 else FRepeatType:=ttSimple;
